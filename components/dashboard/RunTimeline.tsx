...
//
// 수정일: 2026-03-11 (v1.3)
// [v1.3] welcome_commented → welcome_sent / run_duration_sec 제거
// ============================================================
...
export default function RunTimeline({
  ...
  const totalCount = totalPages * PAGE_SIZE; // 표시용 (근사치)
...
export default function RunTimeline({ logs, totalCount, isLoading, hasMore, onLoadMore }: RunTimelineProps) {
  return (
    <div className="card animate-enter">
      <div className="flex items-center justify-between mb-4">
        <p className="section-title mb-0">실행 이력</p>
        <span className="font-stat text-xs text-[#8B949E]">누적 {totalCount.toLocaleString()}건</span>
      </div>
      ...
      {!isLoading && logs.length > 0 && (
        <>
          ...
          {hasMore && (
            <button onClick={onLoadMore} className="w-full mt-3 py-2 text-xs text-[#8B949E] bg-[#0D1117] border border-[#30363D] rounded-lg hover:text-[#E6EDF3] hover:border-[#484F58] transition-all duration-200">
